...
//void outbreak_background(vec p_act, unsigned runid, unsigned my_years, unsigned site_1stAMR,
...
int main(int argc, char *argv[])
{
    ...
    sprintf(temp1, "./mydata2021/ModelNov_recommended%d_regular%d_casual%d_%1.1f_%1.1f_%04d.txt",reconsc,regular,casual,par_rr,par_rcr, Runid); /////char temp[250]
    ofstream fp_output;
    fp_output.open(temp1, ios::trunc);
    sprintf(temp1, "./mydata2021/ModelNov_spreadtree_recommended%d_regular%d_casual%d_%1.1f_%1.1f_%04d.txt",reconsc,regular,casual,par_rr,par_rcr, Runid); /////char temp[250]
    ofstream fp_output_spreadtree;
    fp_output_spreadtree.open(temp1, ios::trunc);
    sprintf(temp1, "./mydata2021/ModelNov_outbreak_recommended%d_regular%d_casual%d_%1.1f_%1.1f_%04d.txt",reconsc,regular,casual,par_rr,par_rcr, Runid); /////char temp[250]
    ofstream fp_outbreak;
    fp_outbreak.open(temp1, ios::trunc);
    //vec p_act =  {0.0978, 0.0098, 0.4379, 0.0763,  0.00312, 0.6339, 0.0188, 0.0048};
	//    p_act =  {0.0859, 0.012, 0.3679, 0.0642,  0.00282, 0.6939, 0.0178, 0.0015};

	//vec p_act =  {0.0852, 0.010, 0.3679, 0.0640,  0.00282, 0.6939, 0.0178, 0.0015};
    vec p_act =  {0.0847642, 0.00940469, 0.469055, 0.0589935, 0.00491175, 0.694099, 0.0125556, 0.00422359}; 
    //vec p_act = {0.0851463, 0.00785235, 0.290079, 0.0742963, 0.00483211, 0.519821, 0.0192287, 0.00240294 };
    ...
    for (unsigned runid = batchsize * (Runid - 1); runid < batchsize * (Runid); runid++)
    {
        ...
        outbreak_background(p_act, runid, myyear, site_1stAMR, output, output_infected, trans_tree,outbreak_d, false, true,
                         ...
                         default_random_engine generator) //recommended screening, casual par_rcr
{
    ...
    uniform_real_distribution<double> unif01(0.0, 1.0);
    uniform_real_distribution<double> gSU(1.0, 3.0);
    //gamma_distribution<double> gSU(3.01, 0.8615); //truncate late.
    ...
    char temp1[100];
    char temp2[100];
    sprintf(temp1, "./mydataNov/Prevalence%03d%02d.txt", runid, 1); /////char temp[250]
    ofstream fp_pre;
    sprintf(temp2, "./mydataNov/Incidence%03d%02d.txt", runid, 1); /////char temp[250]
    ofstream fp_ind;
        ...
        while (Ga.n_elem > lenGa01)
        {
            ...
            if (d == par_day && background)
            {
                ...
                for (unsigned j = 0; j < introInfec.n_elem; j++)
                {
                    ...
                    laststatus(introInfec(j), 3) = 1;
                    pd = gSU(generator) * (sympt == 1) + gAU(generator) * (sympt == 2);
                    pd0 = ceil((pd + 3) * unif01(generator));
                ...
                for (unsigned j = 0; j < introInfec.n_elem; j++)
                {
                    ...
                    laststatus(introInfec(j), 3) = 1;
                    pd = gSU(generator) * (sympt == 1) + gR(generator) * (sympt == 2);
                    pd0 = ceil((pd + 3) * unif01(generator));
...
long long rseed)
{
    default_random_engine generator(rseed);
    std::uniform_real_distribution<double> unif01(0.0, 1.0);
    std::uniform_real_distribution<double> gSU(1.0, 3.0);
    //std::gamma_distribution<double> gSU(3.01, 0.8615); //truncate late.
                ...
                if (today > A.periods(0, 0) && today < A.periods(0, 1))
                {
                    if (unif01(generator) < p_act(3))
                    {
                        if (unif01(generator) < 0.12)
                        {
                            ...
                            pd = round(gSU(generator)); //ceil(2.6+randn*2.24);
                ...
                if (today > A.periods(2, 0) && today < A.periods(2, 1))
                {
                    if (unif01(generator) < p_act(6) * pow(0.10, prot_anal))
                    {
                        if (unif01(generator) < 0.90) // u
                        {
                            ...
                            pd = round(gSU(generator)); //round(6.3+rand*1.4);
                            B.periods(1, 0) = today + 4;
                ...
                if (today > B.periods(2, 0) && today < B.periods(2, 1))
                {
                    if (unif01(generator) < p_act(6) * pow(0.10, prot_anal))
                    {
                        if (unif01(generator) < 0.90) //u
                        {
                            ...
                            pd = round(gSU(generator)); //ceil(6.3+rand*1.4);
                            A.periods(1, 0) = today + 4;
