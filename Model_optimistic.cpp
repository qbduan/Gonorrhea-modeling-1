...
class Nodeman
{
  public:
    ...
    umat periods=zeros<umat>(3,3);//unsigned type uword
...
// rirj_partnership is to handle with regular partnerships, and cicj_partnership is to handle with casual partnerships. gigj_partnership is the handle the group sex within casual partnership.
...
//sch_sex is to schedule the sexual events for each partnership.
mat sch_sex(mat events,long long rseed);
//transAB is to implement the transmission between individuals A and B. 
...
/*outbreak_simulation is to simulate the outbreak. 
p_act is the per act transmission probabalities, where p_act = (p_{o->o},p_{o->u},p_{u->o},p_{o->r},p_{r->o},p_{u->r},p_{r->u},p_{u->u}). 
runid is the simulation order id, which is also related to seed of random number generation.
my_year is the running time after the first importation.
site_1stAMR is supposed to use to control the AMR site of the first imported case, but not used in this project. 
output, output_infected, trans_tree and outbreak_d are used to store output information.
process is to control the output of intermiediate prevalence and incidence.
background is the running year for background strain to get equilibrium. 
reconsc is the strategy of increasing testing to recommended level.
regular (par_rr) is the strategy of contact tracing of regular partners with probablity par_rr.
casual (par_rcr) is the strategy of contact tracing of casual partners with probablity par_rcr.
generator is the random seed for random number generator.
*/
void outbreak_simulation(vec p_act, unsigned runid, unsigned my_years, unsigned site_1stAMR,
                         ...
                         bool reconsc, bool regular, bool casual,double par_rr, double par_rcr, default_random_engine generator); 
...
int main(int argc, char *argv[])
{
    ...
    unsigned batchsize = 10;//For each job task, the program would run for many times (batchsize).
    ...
    //output files 
    char temp1[100];
    sprintf(temp1, "./mydata/OB0618_1frecommended%d_regular%d_casual%d_%1.1f_%1.1f_%04d.txt",reconsc,regular,casual,par_rr,par_rcr, Runid); /////char temp[250]
    ofstream fp_output;
    fp_output.open(temp1, ios::app);
    sprintf(temp1, "./mydata/OB0618_spreadtree_1frecommended%d_regular%d_casual%d_%1.1f_%1.1f_%04d.txt",reconsc,regular,casual,par_rr,par_rcr, Runid); /////char temp[250]
    ofstream fp_output_spreadtree;
    fp_output_spreadtree.open(temp1, ios::app);
    sprintf(temp1, "./mydata/OB0618outbreak_1frecommended%d_regular%d_casual%d_%1.1f_%1.1f_%04d.txt",reconsc,regular,casual,par_rr, par_rcr, Runid); /////char temp[250]
    ofstream fp_outbrak;
    fp_outbrak.open(temp1, ios::app);
	
    //p_act = (p_{o->o},p_{o->u},p_{u->o},p_{o->r},p_{r->o},p_{u->r},p_{r->u},p_{u->u}), and the follwing values are the calibrated results.
    vec p_act =  {0.0978, 0.0098, 0.4379, 0.0763,  0.00312, 0.6339, 0.0188, 0.0048};
    
    unsigned myyear = 5; //5 years run after introduction of imported strain. myyear+20*background+2
    unsigned site_1stAMR = 4;//atoi(argv[2]);
    //double par_rcr = atof(argv[3]);

    for (unsigned runid = batchsize * (Runid - 1); runid < batchsize * (Runid); runid++)
    {
        //runid+(Runid-1)*batchsize;
        vec output(2); // output program stoping information.
        uvec output_infected(1); // output the information of the imported case. 
        umat trans_tree(1, 2); //  output spread tree for the first year.
        umat outbreak_d(2, 1); // output the number of AMR cases and total number of AMR cases 
        output.zeros();
        ...
        outbreak_simulation(p_act, runid, myyear, site_1stAMR, output, output_infected, trans_tree,outbreak_d, false, true,
                            reconsc, regular, casual, par_rr, par_rcr,generator);
        //bool reconsc, bool regular, bool casual,double par_rr, double par_rcr)
...
void outbreak_simulation(vec p_act, unsigned runid, unsigned my_years, unsigned site_1stAMR,
                         vec &output, uvec &output_infected, umat &trans_tree,umat &outbreak_d, bool process, bool background,
                         bool reconsc, bool regular, bool casual, double par_rr, double par_rcr, default_random_engine generator) //recommended screening, casual par_rcr
{
    ...
    gamma_distribution<double> gS(3.01, 0.8615); //truncate.
    // generate the duration of asymptomatic NG infections
    ...
    char temp1[100];
    char temp2[100];
    sprintf(temp1, "Prevalence%03d%02d.txt", runid, 1); 
    ofstream fp_pre;
    sprintf(temp2, "Incidence%03d%02d.txt", runid, 1); 
    ofstream fp_ind;
    if (process)
    {
        fp_pre.open(temp1, ios::app);
        fp_ind.open(temp2, ios::app);
    }

    const int N = 10000;
    unsigned par_day = 5*360;   //5 years run for network
    unsigned back_days = 5 * 360 * (background == true);  // 5 years run for background strain
    unsigned max_day = 360 * my_years + par_day + back_days ; //total days
    
    //If each site is cultured or not. 
    vec culture_rate(3);
    culture_rate(0) = 1;
    culture_rate(1) = 1;
    culture_rate(2) = 1;
    ...
    double U; // [0,1] uniform random number 
    uvec avR(N); //is available for regular partnership
    avR.ones();
    uvec avC(N); //is available for casual partnership
    avC.ones();
    uvec avG(N); //is available for group sex
    avG.fill(100); //people who have group sex, the value is 0/1, and 100 means no group sex
    uvec Gat(N); //when get available for group sex
    Gat.zeros();

    vec acqrate(N); //casual partnership acquisition rate 
    acqrate.zeros();
    ...
    uvec hrskid; //high risk group
    uvec lrskid; //low risk group
    hrskid = find(acqrate > 9);
    lrskid = find(acqrate <= 9);
    std::cout << "Begin program" << std::endl;
    
    // initialization of partnership network
    uvec Rat(N);
    uvec Cat(N);
    uvec Ra = find(avR == 1); //collection of agent ids who is available for regular partnership
    uvec Ca = find(avC == 1); //collection of agent ids who is available for casual partnership
    ...
    // End initialization of partnership network
    ...
    //To calculate and store defined incidence
    ...
    //To calculate and store prevalence
    urowvec all_pre(4);
    urowvec h_pre(4);
    urowvec l_pre(4);
    //To calculate and store prevalence of AMR
    urowvec all_AMR(4);
    urowvec h_AMR(4);
    urowvec l_AMR(4);
    //To calculate and store real incidence
    urowvec all_ind(4);
    urowvec h_ind(4);
    urowvec l_ind(4);
    ...
    {
        if (d >= par_day+back_days && (d - par_day-back_days-1) % 360 == 0)
        {
            std::cout << " " << std::endl;
            std::cout << "Year=" << (d - par_day-back_days-1) / 360 + 1 << endl;
        }
        
        //execute sex acts and perform transimission between each pair of partnership
        uvec ij;
        vec ij0_status(3);
        vec ij1_status(3);
        for (unsigned int n = 0; n < Patsid.n_rows; n++)
        {
            ...
            if ((men[ij(0)].status(1) == 1 && d > men[ij(0)].periods(1, 0)) || (men[ij(0)].status(2) == 1 && d > men[ij(0)].periods(2, 0)) ||
                (men[ij(1)].status(1) == 1 && d > men[ij(1)].periods(1, 0)) || (men[ij(1)].status(2) == 1 && d > men[ij(1)].periods(2, 0)))
            {
                //cout << "Continue other couples" << endl;
                continue;
            }
            // both individuals are healthy and no infections, so schedule sexual events next day .
            ...
            { //TranAB
                ...
                if(d< par_day + back_days+360 && d > par_day + back_days) // output the spreading tree for the first year.
                {
        ...
        for (unsigned int i = 0; i < N; i++)
        {
            ...
            if ((men[i].status(1) == 1 && men[i].periods(1, 1) == d) || (men[i].status(2) == 1 && men[i].periods(2, 1) == d))
            {
                last_testday(i) = d; //different in NG2
                site_treated_all = (men[i].resistance.t() == 0) * 1;
                ...
                else if ((men[i].status(1) == 1 && men[i].resistance(1) == 1) || (men[i].status(2) == 1 && men[i].resistance(2) == 1))
                {
                    site_treated_all.ones();
                    detectAMR = true;
                }
            ...
            if (!detected1st_treat && !detected1st_test && detectAMR)
            {
                ...
                info_1stdetected(9) = d - par_day - back_days;
                all_AMR.zeros();
                //total_AMR.zeros();
            ...
            for (unsigned int s = 0; s < 3; s++)
            {
                if (site_treated_all(s) == 1 && men[i].status(s) > 0) //&& men[i].resistance(s) == 0) //whether the screending day should change
                {
                    men[i].periods(s, 0) = 0;
                    men[i].periods(s, 1) = d;
                    men[i].periods(s, 2) = d + 7;
                }
                
            }
            // Implemention of contact tracing
            if (regular)
            {
                trace_partners.resize(1);
            ...
            if (casual)
            {
                ...
                if (detectAMR && men[i].behaviour != 1 && men[i].GhisPar.n_rows > 1 )
                {
            ...
            for (unsigned int s = 0; s < 3; s++)
            {
                if (men[i].periods(s, 1) == d)
                {
                    men[i].status(s) = -1;
                    men[i].periods(s, 1) = 0;
                    men[i].resistance(s) = 0;
                }
                else if (men[i].periods(s, 2) == d)
                {
                    men[i].status(s) = 0;
                    men[i].periods(s, 2) = 0;
                }
        ...
        while (Ga.n_elem > lenGa01)
        {
            ...
            while (Ra.n_elem > 1 || Ca.n_elem > 1)
            {
                ...
                if (Ra.n_elem > 1 && U < prob_re)
                {
                    ...
                    //if(outputpartnerships){
                    //fprintf(fp_parnterhsips, "%d %d %d %d %d \n", TypeM(ri, rj),ri,rj,d, StopDayR(ri, rj) );
                    //fprintf(fp_parnterhsips, "%d %d %d %d %d \n", TypeM(ri, rj),rj,ri,d, StopDayR(ri, rj) );
                    //}
                    avR(ri) = 0;
                    avR(rj) = 0;
                }
                else if (Ca.n_elem > 1 && U > prob_re)
                {
                    ...
                    //if(outputpartnerships){
                    //fprintf(fp_parnterhsips, "%d  %d %d %d %d \n", TypeM(ci, cj),ci,cj,d, StopDayC(ci, cj) );
                    //fprintf(fp_parnterhsips, "%d  %d %d %d %d \n", TypeM(ci, cj),cj,ci,d, StopDayC(ci, cj) );
                    //}
                    avC(ci) = 0;
                    avC(cj) = 0;
                }
            ...
            if (d == par_day && background)
            {
                ...
                for (unsigned j = 0; j < introInfec.n_elem; j++)
                {
                    ...
                    men[introInfec(j)].periods(0, 2) = d + 11 + pd - pd0;
                }
                ...
                for (unsigned j = 0; j < introInfec.n_elem; j++)
                {
                    ...
                    men[introInfec(j)].periods(1, 2) = d + 11 + pd - pd0;
                }
                ...
                for (unsigned j = 0; j < introInfec.n_elem; j++)
                {
                    ...
                    men[introInfec(j)].periods(2, 2) = d + 11 + pd - pd0;
                }
                //if (Debuge)
                //{
            ...
            if (d == par_day + back_days)
            {
                ...
                if (men[infect_1st].status(1) > 0)
                {
                    ...
                    men[infect_1st].periods(1, 2) = d + 11 + pd - pd0;
                }
                if (men[infect_1st].status(2) > 0)
                {
                    ...
                    men[infect_1st].periods(2, 2) = d + 11 + pd - pd0;
                }
...
Nodeman::Nodeman(unsigned int id, long long rseed)
{
    ...
    if (this->acqrate > 9)
    {
        ...
        U=unif01(generator);
        this->testday = (360+360*ceil(unif01(generator)))*(U<0.05)
    ...
    {
        this->testday = (360+360*ceil(unif01(generator)))*(U<0.10)
        +360*(U>=0.10 && U<1);
    }
...
void Nodeman::Initializer(unsigned int id,long long rseed)
{
    ...
    if (this->acqrate > 9)
    {
        double testdist_hr[4]={0.05,0.49,0.71,1.0};
        U=unif01(generator);
        testday =(480+360*ceil(unif01(generator)))*(U<0.05)
        +360*(U>=0.05 && U<0.49) +180*(U>=0.49 && U<0.71)
        +120*(U>=0.71 && U<0.89) + 90*(U>=0.89 && U<1);
    }
    else if(this->acqrate>0 && this->acqrate<1)
...
long long rseed)
{
    ...
    temPartner(0)=mri.name;temPartner(1)=mrj.name;temPartner(2)=today;temPartner(3)=stopDayR;
    mri.RhisPar.insert_rows(mri.RhisPar.n_rows, temPartner);
    //temPartner = {mrj.name, mri.name, today, stopDayR};
    temPartner(0)=mrj.name;temPartner(1)=mri.name;temPartner(2)=today;temPartner(3)=stopDayR;
...
long long rseed)
{
    ...
    temPartner(0)=mci.name; temPartner(1)=mcj.name; temPartner(2)=today; temPartner(3)=stopDayC;
    mci.ChisPar.insert_rows(mci.ChisPar.n_rows, temPartner);
    //temPartner = {mcj.name, mci.name, today, stopDayC};
    temPartner(0)= mcj.name; temPartner(1)= mci.name; temPartner(2)=today; temPartner(3)=stopDayC;
...
long long rseed)
{
    default_random_engine generator(rseed);
    std::uniform_real_distribution<double> unif01(0.0, 1.0);
    //std::uniform_real_distribution<double> gS(2.0, 4.0);
